...
class NewsSpider(scrapy.Spider):
	...
	name = 'newsspider'
	start_urls = ['https://english.alarabiya.net/News/middle-east/archive.news.html']
	...
	def parse(self, response):
		...
		for news in response.css(SET_SELECTOR):
			...
			self.subdomain.append('middleeast')
			self.downloadate.append(self.now)
			self.type.append('archive')

			print('alarabiya', 'middleeast')
		...
		if next_page:
			...
			self.out.to_csv('../data/alarabiya_middleeast_{}.csv'.format(self.now))
