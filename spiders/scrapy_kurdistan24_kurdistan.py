...
class NewsSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		for news in response.css(SET_SELECTOR):
			...
			self.subdomain.append('kurdistan')
			self.downloadate.append(self.now)
			self.type.append('archive')
			i += 1

			print('kurdistan24', 'kurdistan')
		...
		if self.page_number < 300:  # Can set 100 to any number. This represents the number of pages we want to scrape.
			...
			self.out.to_csv('../data/kurdistan24_kurdistan_{}.csv'.format(self.now))
